...
public class HttpProtocolHandler {
	...
	private HttpProtocolHandler() {
		...
		((PoolingHttpClientConnectionManager) connectionManager)
				.setMaxTotal(defaultMaxTotalConn);
		((PoolingHttpClientConnectionManager) connectionManager)
				.setDefaultMaxPerRoute(defaultMaxConnPerHost);
		// socketConfig);
	...
	public HttpResponse execute(HttpRequest request, String strParaFileName,
		...
		if (request.getMethod().equals(HttpRequest.METHOD_GET)) {
			...
			for (int i = 0; i < request.getParameters().length; i++) {
				...
				builder.addPart(request.getParameters()[i].getName(), comment);
				// parts.add(new
				// StringPart(request.getParameters()[i].getName(),
				// request.getParameters()[i].getValue(), charset));
			}
			// 增加文件参数，strParaFileName是参数名，使用本地文件
			// parts.add(new FilePart(strParaFileName, new FilePartSource(new
			// File(strFilePath))));
			FileBody bin = new FileBody(new File(strFilePath));
			builder.addPart(strParaFileName, bin);

			// 设置请求体
			// ((PostMethod) method).setRequestEntity(new
			// MultipartRequestEntity(parts.toArray(new Part[0]), new
			// HttpMethodParams()));
		...
		method.addHeader(new BasicHeader("User-Agent", "Mozilla/4.0"));
		...
		try {
			...
			if (request.getResultType().equals(HttpResultType.STRING)) {
				...
				response.setStringResult(EntityUtils.toString(entity1));
			} else if (request.getResultType().equals(HttpResultType.BYTES)) {
				// response.setByteResult(method.getResponseBody());
				response.setByteResult(EntityUtils.toByteArray(entity1));
			}
			// response.setResponseHeaders(method.getResponseHeaders());
			response.setResponseHeaders(response1.getAllHeaders());
